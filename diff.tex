...
%DIF DEL .\Math-7081.tex          Mon Apr 24 22:31:08 2023
%DIF ADD .\Math-7081 - Copy.tex   Fri Apr 28 21:39:13 2023
...
\begin{document}
	...
	\address{%
		\addr{\affilnum{1}}{Northeast Yucai School, Zhenxing St. 9-5, Heping District, Shenyang, Liaoning, P.R.China. Email: felix\_wzj@yahoo.com}
		\corraddr{Department of Mathematics, College of Sciences, Northerneastern University, Shenyang, Liaoning, P.R.China. Email: shaoxinhui@mail.neu.edu.cn}
	}
	...
	\keywords{
		\textbf{automatic prover, theorem prover, algorithm, category theory, automatic deduction, automatic reasoning}
		\newline
	...
	To begin with, we use notation \(\mathcal{C}\) to demonstrate a category and use \DIFdelbegin \DIFdel{script}\DIFdelend \(\mathcal{F}\) to present a functor. \(\mathcal{U}\) stands for the Grothendieck universe selected and \DIFdelbegin \DIFdel{\(\mathcal{F}\mathit{o}\mathit{r}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\mathcal{F}_r\) }\DIFaddend stands for the forgetful functor. A map is noted in one of the two following forms:
	...
	\DIFaddbegin \DIFadd{We use symbol \(On\) as the ordinal class composed by ordinals \mbox{%DIFAUXCMD
\cite{Li2019} }\hskip0pt%DIFAUXCMD
such as \(\{\emptyset\}\) and infinite ordinal \(\omega\) which is defined as the inductive set supported by the Axiom of Infinity in ZFC. Note that \(On\) is a proper class instead of a set, otherwise it will lead to the Burali-Forti paradox.
	}

	\DIFadd{Additionally, we use \(S_p\) with \(S\) being a set and \(p\) being a statement about elements in \(S\) to represent subset \(\{x\in S|p(x)\equiv T\}\). For example, notation \(\mathbb{Z}_{>1}\) represents the set of integers greater than 1. Moreover, we utilized \(On_{\geq n}\) to represent ordinals not smaller than \(n\).
	}

	\DIFaddend \subsection{Pre-Knowledge}

	The definitions and theories presented in the paper utilizes the concepts of the category theory \cite{Li2019}\DIFaddbegin \DIFadd{, the ordinal theory \mbox{%DIFAUXCMD
\cite{Li2019} }\hskip0pt%DIFAUXCMD
}\DIFaddend and axiomatic set theory \cite{Li2019}. Therefore, readers are recommended to review understandings and conclusions about \DIFdelbegin \DIFdel{the two }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend concepts before going over the paper.
	...
	\begin{equation}
		\mathcal{C}=\text{Sym}(\mathcal{C})\DIFdelbegin %DIFDELCMD < \sqcup %%%
\DIFdelend \DIFaddbegin \DIFadd{\cup }\DIFaddend \text{Not}(\mathcal{C})\DIFdelbegin %DIFDELCMD < \sqcup %%%
\DIFdelend \DIFaddbegin \DIFadd{\cup }\DIFaddend \text{Bra}(\mathcal{C})
	\end{equation}

	The \DIFdelbegin \DIFdel{three forms a partition of the big category }\DIFdelend \DIFaddbegin \DIFadd{formula above specifies that the meta category \(\mathcal{C}\) is the union set of the symbol subcategory, the notation subcategory and the bracket subcategory. It should be noted that though we will separately manipulate on \(\text{Sym}(\mathcal{C})\), \(\text{Not}(\mathcal{C})\) and \(\text{Bra}(\mathcal{C})\) afterwards, the annotation \(\mathcal{C}\) is still used to represent these subcategories serve for a common context}\DIFaddend .
